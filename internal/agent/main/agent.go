// Package mainagent implements the QQ assistant agent driven by a todo queue:
...
import (
	...
	agenttool "miss-raspberry-agent/internal/tool"
...
func NewMainAgent(ctx context.Context, chatModel model.BaseChatModel, sender messaging.Sender, history messaging.HistoryProvider) (*MainAgent, error) {
	...
	tools := []tool.BaseTool{
		...
		schedule.NewScheduleTaskTool(sched.Store(), a.currentSource),
		qq.NewQQMessageSender(sender),
		qq.NewQQMessageGetter(history),
		todo.NewTodoListTool(a.todo),
	}
	...
	chatModel = llm.StrictProviderChatModel(chatModel)
...
// polls.
func (a *MainAgent) Queue() *todo.Store {
	return a.todo
}
