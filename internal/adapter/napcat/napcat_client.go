...
var _ messaging.Sender = (*NapcatClient)(nil)
var _ messaging.HistoryProvider = (*NapcatClient)(nil)
...
func (c *NapcatClient) SetTodoList(todo *todo.Store) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.todoList = todo
}
...
// SendMessage sends a private or group message (called by the agent). It implements
// messaging.Sender by translating the application-level target into the internal
// NapCat message shape.
func (c *NapcatClient) SendMessage(_ context.Context, target messaging.Target, content string) error {
	msg := Message{
		Content: content,
	}
	if target.Type == messaging.TargetTypeGroup {
		msg.GroupID = target.ID
	} else {
		msg.UserID = target.ID
	}
...
func (c *NapcatClient) GetMessageHistory(ctx context.Context, target messaging.Target, beforeSeq int64, count int) ([]messaging.HistoryMessage, error) {
	action := "get_group_msg_history"
	params := zero.Params{
		"group_id": target.ID,
		"count":    count,
	}
	if target.Type == messaging.TargetTypePrivate {
...
func ParseHistoryResponse(data gjson.Result) []messaging.HistoryMessage {
	...
	messages := make([]messaging.HistoryMessage, 0, len(messagesResult.Array()))
	for _, item := range messagesResult.Array() {
		seq := item.Get("message_seq").Int()
		if seq == 0 {
			seq = item.Get("message_id").Int()
		}
		messages = append(messages, messaging.HistoryMessage{
