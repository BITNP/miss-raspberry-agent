...
func GetMessageHistory(ctx context.Context, provider messaging.HistoryProvider, in *QQMessageGetterInput) (*QQMessageGetterOutput, error) {
	if err := validateGetterInput(in); err != nil {
		return nil, err
	}

	target := messaging.Target{
		Type: messaging.TargetType(in.TargetType),
		ID:   in.TargetId,
	}

	var (
		raw []messaging.HistoryMessage
		err error
	)
	if in.StartTime > 0 || in.EndTime > 0 {
		raw, err = fetchInTimeRange(ctx, provider, target, in)
	} else {
		raw, err = provider.GetMessageHistory(ctx, target, 0, normalizeCount(in.Count))
	}
	if err != nil {
...
func fetchInTimeRange(ctx context.Context, provider messaging.HistoryProvider, target messaging.Target, in *QQMessageGetterInput) ([]messaging.HistoryMessage, error) {
	...
	for {
		page, err := provider.GetMessageHistory(ctx, target, nextSeq, historyPageSize)
		if err != nil {
			return nil, err
		}
		if len(page) == 0 {
	...
	selected := make([]messaging.HistoryMessage, 0, len(all))
	for _, m := range all {
...
func buildHistoryOutput(raw []messaging.HistoryMessage) *QQMessageGetterOutput {
...
func sortHistoryNewestFirst(messages []messaging.HistoryMessage) {
...
func NewQQMessageGetter(provider messaging.HistoryProvider) tool.BaseTool {
