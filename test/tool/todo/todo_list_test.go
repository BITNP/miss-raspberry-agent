...
func TestStoreAddListComplete(t *testing.T) {
	...
	first := store.Add(todo.Item{Content: "回复用户A", Source: "私聊(用户QQ=1)"})
	second := store.Add(todo.Item{Content: "回复群B", Source: "群聊(群号=2)"})
...
func TestVersionIncrementsOnAdd(t *testing.T) {
	...
	store.Add(todo.Item{Content: "one"})
	store.Add(todo.Item{Content: "two"})
...
func TestStorePersistsContext(t *testing.T) {
	...
	store.Add(todo.Item{Content: "无上下文"})
...
func TestTodoListToolActions(t *testing.T) {
	...
	out, err := todo.RunTodoList(context.Background(), store, &todo.TodoListInput{Action: "add", Content: "任务一"})
	if err != nil {
		t.Fatalf("add: %v", err)
	}
	if len(out.Items) != 1 {
	...
	out, err = todo.RunTodoList(context.Background(), store, &todo.TodoListInput{Action: "list"})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	...
	out, err = todo.RunTodoList(context.Background(), store, &todo.TodoListInput{Action: "complete", ID: id})
	if err != nil {
...
func TestTodoListToolValidation(t *testing.T) {
	...
	cases := []struct {
		name string
		in   *todo.TodoListInput
	}{
		{"unknown action", &todo.TodoListInput{Action: "delete"}},
		{"add without content", &todo.TodoListInput{Action: "add"}},
		{"complete without id", &todo.TodoListInput{Action: "complete"}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := todo.RunTodoList(context.Background(), store, tc.in); err == nil {
