...
import (
	...
	"miss-raspberry-agent/internal/tool/qq"
)
...
type fakeHistoryProvider struct {
	messages []messaging.HistoryMessage
	mu       sync.Mutex
	calls    []historyCall
}
...
func (f *fakeHistoryProvider) GetMessageHistory(ctx context.Context, target messaging.Target, beforeSeq int64, count int) ([]messaging.HistoryMessage, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.calls = append(f.calls, historyCall{target: target, beforeSeq: beforeSeq, count: count})

	var result []messaging.HistoryMessage
...
func TestGetMessageHistoryByCount(t *testing.T) {
	...
	out, err := qq.GetMessageHistory(context.Background(), provider, &qq.QQMessageGetterInput{
		TargetType: "group",
		TargetId:   123,
		Count:      3,
	})
...
func TestGetMessageHistoryKeepsOnlyTextMessages(t *testing.T) {
	provider := &fakeHistoryProvider{messages: []messaging.HistoryMessage{
...
func TestGetMessageHistoryByTimeRangePaginates(t *testing.T) {
	...
	msgs := make([]messaging.HistoryMessage, 0, 120)
	for seq := int64(1119); seq >= 1000; seq-- {
		msgs = append(msgs, messaging.HistoryMessage{
			MessageID:  seq,
			MessageSeq: seq,
			Time:       seq,
			UserID:     7,
...
func TestGetMessageHistoryValidation(t *testing.T) {
	...
	cases := []struct {
		name string
		in   *qq.QQMessageGetterInput
	}{
		{"unknown target_type", &qq.QQMessageGetterInput{TargetType: "channel", TargetId: 1}},
		{"missing target_id", &qq.QQMessageGetterInput{TargetType: "group", TargetId: 0}},
		{"reversed time range", &qq.QQMessageGetterInput{TargetType: "group", TargetId: 1, StartTime: 200, EndTime: 100}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := qq.GetMessageHistory(context.Background(), provider, tc.in); err == nil {
...
func TestSendMessageRoutesToPrivateAndGroup(t *testing.T) {
	...
	if _, err := qq.SendMessage(context.Background(), client, &qq.QQMessageSenderInput{
		TargetId: 123,
		Content:  "hello",
	}); err != nil {
...
func TestSendMessageRejectsUnknownTargetType(t *testing.T) {
	client := napcat.NewClient(nil)
	_, err := qq.SendMessage(context.Background(), client, &qq.QQMessageSenderInput{
		TargetId:   1,
		TargetType: "channel",
		Content:    "x",
	})
...
func historyMessages(t *testing.T, n int) []messaging.HistoryMessage {
	t.Helper()
	msgs := make([]messaging.HistoryMessage, 0, n)
	for i := n; i >= 1; i-- {
		seq := int64(i + 100)
		msgs = append(msgs, messaging.HistoryMessage{
			MessageID:  seq,
